...
resource "aws_subnet" "prod_subnet_one" {
  ...
  tags = {
    ...
    Region  = "Subbnet in Region: ${data.aws_region.current.description}"
  }
}

resource "aws_subnet" "prod_subnet_two" {
  vpc_id            = data.aws_vpc.my_prod_vpc.id
...
output "data_my_prod_vpc_id" {
  value = data.aws_vpc.my_prod_vpc.id
}
output "data_my_prod_vpc_cidr" {
